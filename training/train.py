...
from .model import ModelConfig, TinyGPT, greedy_decode, lm_loss
...
class TrainConfig:
    ...
    # logging
    eval_every: int = 250
    eval_max_examples: int = 4096          # per partition, fixed random subset (seeded) for speed
...
@torch.no_grad()
def evaluate(model, enc: Encoded, vocab: Vocab, device, batch=1024, per_cell=False):
    model.eval()
...
def run(cfg: TrainConfig) -> Path:
    ...
    parts = load_partitions(cfg.view, cfg.split, cfg.condition, derived_base, splits_base)
    ...
    for p, e in enc.items():
        n = e.tokens.shape[0]
        sel = np.sort(rng.choice(n, size=min(n, cfg.eval_max_examples), replace=False))
    ...
    meta = dict(config=dataclasses.asdict(cfg), model=dataclasses.asdict(mcfg), n_params=model.n_params(),
    ...
    with open(out / "metrics.jsonl", "w") as mf:
        for step in range(1, cfg.steps + 1):
            ...
            sched.step()
            if step in ckpts:
                torch.save(dict(step=step, model=model.state_dict(), model_config=dataclasses.asdict(mcfg)),
